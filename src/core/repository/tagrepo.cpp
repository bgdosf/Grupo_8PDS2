...
std::vector<Tag> TagRepo::getAllTags() {
    ...
    std::string sql =
        "SELECT * FROM tag WHERE username='" + _u.username() + "';";
...
Tag* TagRepo::getTagByName(std::string tagname) {
    std::string sql = "SELECT * FROM tag WHERE name='" + tagname +
    ...
    Tag* t = new Tag(queryResult[0]["name"], stoi(queryResult[0]["id"]),
                     queryResult[0]["username"]);
    return t;
}

Tag* TagRepo::getTagById(int id) {
    std::string sql = "SELECT * FROM tag WHERE id=" + std::to_string(id) +
                      " AND username='" + _u.username() + "';";
    std::vector<std::map<std::string, std::string>> queryResult =
        sqlselect(sql);
    if (queryResult.size() == 0) {  // caso a fila de resultados esteja vazia
...
Tag* TagRepo::createTag(std::string tagname) {
    std::string sql = "INSERT INTO tag(name, username) values('" + tagname +
                      "', '" + _u.username() + "');";
...
Tag* TagRepo::updateTag(Tag t, std::string tagname) {
    std::string sql = "UPDATE tag SET name='" + tagname +
                      "' WHERE id=" + std::to_string(t.id()) +
                      " AND username='" + _u.username() + "';";
    if (execSql(sql) == 0) return nullptr;
