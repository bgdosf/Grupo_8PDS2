...
Repo::Repo() { sqlite3_open(getenv("DATABASE"), &db); }

Repo::~Repo() { sqlite3_close(db); }
...
std::vector<std::map<std::string, std::string>> Repo::sqlselect(
    std::string command) const {
  sqlite3_stmt *statement;
  std::vector<std::map<std::string, std::string>> result;

  // Consulta
  if (sqlite3_prepare_v2(db, command.c_str(), -1, &statement, nullptr) !=
      SQLITE_OK) {
